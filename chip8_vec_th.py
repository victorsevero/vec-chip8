...
class Chip8:
    ...
    def fetch_opcode(self) -> th.ShortTensor:
        ...
        # swap bytes
        # TODO: probably better to do that when loading the program
        return (
    ...
    def execute_opcode(self, opcode: th.ShortTensor):
        # Extracting nibbles from opcode
        nnn = opcode & 0x0FFF
        n = opcode & 0x000F
        x = (opcode & 0x0F00) >> 8
        y = (opcode & 0x00F0) >> 4
        kk = opcode.type(th.uint8)

        # TODO: maybe try checking all operations at once and call them after
        ...
        if (opcode & 0xF000 == 0x5000).any():
            opcode_idxs = th.nonzero(opcode & 0xF000 == 0x5000).squeeze(1)
        ...
        if (opcode & 0xF00F == 0x8004).any():
            ...
            sum_value = (
                self.v[opcode_idxs, x[opcode_idxs]].int()
                + self.v[opcode_idxs, y[opcode_idxs]].int()
            )
            carry = th.where(sum_value > 0xFF, 1, 0).type(th.uint8)
            self.v[opcode_idxs, x[opcode_idxs]] = sum_value.type(th.uint8)
        ...
        if (opcode & 0xF000 == 0xD000).any():
            ...
            mask_y = (
                base_range_y[th.newaxis, :] < clipped_y_size[:, th.newaxis]
            )
            ...
            mask_x = (
                base_range_x[th.newaxis, :] < clipped_x_size[:, th.newaxis]
            )
        ...
        if (opcode & 0xF0FF == 0xE0A1).any():
            ...
            self.pc[opcode_idxs] += th.where(
                self.keys[
                    opcode_idxs, self.v[opcode_idxs, x[opcode_idxs]].int()
                ]
                == 0,
                2,
                0,
        ...
        if (opcode & 0xF0FF == 0xF00A).any():
            ...
            ) & th.any(self.keys[opcode_idxs, :-1], axis=1)
            just_released = (self.pressed_key[opcode_idxs] != KEY_COUNT) & (
                self.keys[opcode_idxs, self.pressed_key[opcode_idxs].int()]
                == 0
            )
            ...
            self.v[opcode_idxs, x[opcode_idxs]] = th.where(
                just_released,
                self.pressed_key[opcode_idxs],
                self.v[opcode_idxs, x[opcode_idxs]],
            ).type(th.uint8)
        ...
        if (opcode & 0xF0FF == 0xF029).any():
            ...
            self.i[opcode_idxs] = self.v[opcode_idxs, x[opcode_idxs]].int() * 5
