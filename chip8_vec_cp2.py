...
class Chip8:
    def __init__(self, n_emulators, seed=None):
        ...
        self.opcode_formats = cp.array(list(self.opcode_methods.keys()))
    ...
    def execute_opcode(self, opcode: cp.ndarray):
        ...
        self.n = (opcode & 0x000F).astype(cp.uint8)
        self.x = (opcode & 0x0F00) >> 8
        self.y = (opcode & 0x00F0) >> 4
        self.kk = opcode.astype(cp.uint8)

        opcode_formats = cp.vstack(
            (opcode, opcode & 0xF000, opcode & 0xF00F, opcode & 0xF0FF)
        )
        exec_opcodes = cp.unique(
            opcode_formats[cp.isin(opcode_formats, self.opcode_formats)]
        )
    ...
    def op_8xy4(self, opcode):
        ...
        sum_value = (
            self.v[opcode_idxs, self.x[opcode_idxs]].astype(cp.uint16)
            + self.v[opcode_idxs, self.y[opcode_idxs]]
        )
        carry = cp.where(sum_value > 0xFF, 1, 0).astype(cp.uint8)
        self.v[opcode_idxs, self.x[opcode_idxs]] = sum_value.astype(cp.uint8)
    ...
    def op_Dxyn(self, opcode):
        ...
        clipped_y_size = cp.minimum(SCREEN_HEIGHT - vy, self.n[opcode_idxs])
        ...
        mask_y = base_range_y[cp.newaxis, :] < clipped_y_size[:, cp.newaxis]

        max_size_x = 8
        base_range_x = cp.arange(max_size_x)
        ranges_x = cp.repeat(base_range_x[cp.newaxis, :], n_ems, axis=0)
        mask_x = base_range_x[cp.newaxis, :] < clipped_x_size[:, cp.newaxis]
    ...
    def op_Fx0A(self, opcode):
        ...
        just_pressed = (self.pressed_key[opcode_idxs] == KEY_COUNT) & cp.any(
        ...
        self.pc[opcode_idxs] -= cp.where(just_released, 0, 2).astype(cp.uint16)

        self.v[opcode_idxs, self.x[opcode_idxs]] = cp.where(
            just_released,
            self.pressed_key[opcode_idxs],
            self.v[opcode_idxs, self.x[opcode_idxs]],
        ).astype(cp.uint8)
    ...
    def op_Fx55(self, opcode):
        ...
        mask = range_x < self.x[opcode_idxs][:, cp.newaxis] + 1
